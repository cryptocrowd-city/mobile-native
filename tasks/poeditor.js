    ...
    request(options, (error, response, body) => {
      ...
      if (response && response.statusCode) {
        ...
        if (json.response.status !== 'success') {
          reject(
            new Error(json.response.message || JSON.stringify(json.response)),
          );
          return;
        }
        resolve(json.result);
...
const downloadLocale = async (locale, force = false) => {
  ...
  if (locale === 'en' && !force) {
    throw new Error("You can't export the english locale");
  }
  ...
  writeFileSync(destination, fileContent.replace(/\\n/gm, ' '));
...
const listLocales = async () => {
  const {response, result} = JSON.parse(
    await req('https://api.poeditor.com/v2/languages/list', 'post', {
      api_token: argv['poeditor-key'],
      id: argv['poeditor-id'] || PROJECT_ID,
    }),
  );
  ...
  try {
    ...
    switch (argv._[0]) {
      ...
      case 'download':
        if (!argv._[1]) {
          throw new Error(
            'missing locale parameter: yarn locale download [locale]\nExample yarn locale download es',
          );
        }
